...
namespace OutlineCamera
{
    ...
    public class OutlineEffect : MonoBehaviour
    {
        ...
        {
            m_instance = this;
            lineColor0 = Parameters.outlineColor;
        }
        ...
        void DrawRenderersBasedOnOutlineMaterials(Outline outline, Material sharedMaterial)
        {
            ...
            if (sharedMaterial.mainTexture != null)
            {
                ...
                {
                    if (!(materialInList.mainTexture == sharedMaterial.mainTexture))
                    {
                        continue;
                    }

                    if ((outline.eraseRenderer && materialInList.color == outlineEraseMaterial.color) ||
                        (materialInList.color == GetMaterialFromID(outline.color).color))
                    {
                        material = materialInList;
                    }
                }
                ...
                if (material == null)
                {
                    ...
                    {
                        material = new Material(GetMaterialFromID(outline.color));
                    }
            ...
            else
            {
                if (outline.eraseRenderer)
                    ...
                    material = GetMaterialFromID(outline.color);
            }

            if (backfaceCulling)
            {
                material.SetInt("_Culling", (int)CullMode.Back);
            }
            else
            {
                material.SetInt("_Culling", (int)CullMode.Off);
            }
            ...
            if (meshFilter)
            {
                ...
                {
                    commandBuffer.DrawRenderer(outline.GetComponent<Renderer>(), material, i, 0);
                }
            }
            SkinnedMeshRenderer skinnedMeshRenderer = outline.GetComponent<SkinnedMeshRenderer>();
            if (skinnedMeshRenderer)
            {
                for (int i = 1; i < skinnedMeshRenderer.sharedMesh.subMeshCount; i++)
                {
                    commandBuffer.DrawRenderer(outline.GetComponent<Renderer>(), material, i, 0);
                }
            }
        ...
        void ShittyCode()
        {
            foreach(Outline outline in outlines)
            {
                ...
                if (!(layerMask == (layerMask | (1 << outline.originalLayer))))
                {
                    return;
                }

                foreach (Material sharedMaterial in outline.Renderer.sharedMaterials)
                {
                    DrawRenderersBasedOnOutlineMaterials(outline, sharedMaterial);
                }
            }
        }
        ...
        float getScalingFactorFromScreenSize()
        {
            return Screen.height / 360.0f;
        }
        ...
        public void UpdateMaterialsPublicProperties()
        {
            ...
            {
                scalingFactor = getScalingFactorFromScreenSize();
            }
